...
public class SortedTableMap<K, V> extends AbstractSortedMap<K, V> {
  ...
  entry with a key greater than or equal to k (or else index high+1, by
  convention).
*/
  private int findIndex(K key, int low, int high) {
    ...
    // found exact match
      ...
      return findIndex(key, low,
                       mid - 1); // answer is left of mid (or possibly mid)
    else
