...
// el libro utiliza hashmap de java.util para la implementacion
import java.util.HashMap;
import java.util.List;
import java.util.Map;

public class HashMultimap<K, V> {
  java.util.Map<K, List<V>> map = new HashMap<K, List<V>>(); // the primary map
  int total = 0;
  ...
  Iterable<Map.Entry<K, V>> entries() {
    ...
    for (Map.Entry<K, List<V>> secondary : map.entrySet()) {
      ...
      for (V value : secondary.getValue())
        result.add(new java.util.AbstractMap.SimpleEntry<K, V>(key, value));
    }
    return result;
  }
}
