...
public class TreeMap<K, V> extends AbstractSortedMap<K, V> {
  ...
  protected static class BalanceableBinaryTree<K, V>
    ...
    /**
     * A specialized version of Node that includes an auxiliary int for
    ...
    @Override
    protected Node<Entry<K, V>>
    createNode(Entry<K, V> e, Node<Entry<K, V>> parent, Node<Entry<K, V>> left,
     ...
     * parent/grandparent. Returns the Position that becomes the root of the
  ...
  /**
     Recomputes the height of the given position based on its
          children's heights.
   */
  ...
  /**
     Returns
          whether a position has balance factor between −1 and 1 inclusive.
   */
  ...
  /**
   * Returns entry of p's nearest ancestor with key that is smaller/larger than
   * p. Returns null if no such ancestor exists.
   */
  protected Entry<K, V> oneSidedAncestor(Position<Entry<K, V>> p,
                                         boolean smaller) {
    while (!isRoot(p)) {
      if (smaller ==
          (p ==
           right(parent(
               p)))) // parent has smaller key if p is right child of parent
   ...
   * already in the map, this replaces the previous value with the new one and
  ...
  public V remove(K key) {
    ...
    if (isExternal(p)) {  // key not found
      ...
      if (isInternal(left(p)) &&
          isInternal(right(p))) { // both children are internal
  ...
  public Entry<K, V> lowerEntry(K key) {
    ...
    if (isInternal(p) && isInternal(left(p))) // if p has left subtree,
      return treeMax(left(p))
          .getElement(); // predecessor is max of that subtree
    return oneSidedAncestor(
        p, true); // otherwise predecessor is a SMALLER ancestor
  }
  ...
  public Entry<K, V> higherEntry(K key) {
    ...
    return oneSidedAncestor(p,
                            false); // otherwise successor is a LARGER ancestor
  }
  ...
  private void subMapRecur(K fromKey, K toKey, Position<Entry<K, V>> p,
                           ArrayList<Entry<K, V>> buffer) {
    if (isInternal(p))
      if (compare(p.getElement(), fromKey) < 0)
        // p's key is less than fromKey, so any relevant entries are to the
        // right
        subMapRecur(fromKey, toKey, right(p), buffer);
      else {
        subMapRecur(fromKey, toKey, left(p),
                    buffer);                      // first consider left subtree
        if (compare(p.getElement(), toKey) < 0) { // p is within range
          buffer.add(p.getElement()); // so add it to buffer, and consider
          subMapRecur(fromKey, toKey, right(p),
                      buffer); // right subtree as well
        }
      }
  }
  ...
  private void dumpRecurse(Position<Entry<K, V>> p, int depth) {
    String indent =
        (depth == 0 ? "" : String.format("%" + (2 * depth) + "s", ""));
    if (isExternal(p))
